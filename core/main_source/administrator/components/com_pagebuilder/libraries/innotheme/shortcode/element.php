...
class IG_Pb_Element extends IG_Pb_Common {
    ...
    public function element_items_extra() {
        ...
        $appearing_animation = array(
            'name'      => JText::_('Appearing Animation'),
            'id'        => 'appearing_animation',
            'type'      => 'select',
            'std'       => '0',
            'options'   => JSNPagebuilderHelpersType::getAnimation(),
            'tooltip'   => JText::_('Add Animation to Element'),
            'has_depend' => '1'
        );
        $appearing_animation_speed = array(
            'name'          => JText::_('Appearing Animation Speed'),
            ...
            'tooltip'   => JText::_('Add Animation Speed to Element')
        );
...
//     					$css_suffix
    ...
    public function element_in_pgbldr($content = '', $shortcode_data = '', $el_title = '') {
        ...
        $buttons = array(
            'edit' => '<a href="#" onclick="return false;" title="'.JText::_('Edit element').'" data-shortcode="' . $shortcode . '" class="element-edit"><i class="icon-pencil"></i></a>',
            'clone' => '<a href="#" onclick="return false;" title="'.JText::_('Duplicate element').'" data-shortcode="' . $shortcode . '" class="element-clone"><i class="icon-copy"></i></a>',
            'delete' => '<a href="#" onclick="return false;" title="'.JText::_('Delete element').'" class="element-delete"><i class="icon-trash"></i></a>'
        );
        ...
        if (empty($content)) {
            if (!$is_sub_element)
                ...
                if(isset($exception['item_text'])){
                	...
                	$content = JText::_(ucfirst($parent_shortcode)) ." ". JText::_('Item') ." PB_INDEX_TRICK";
                }
            }
        }
        ...
        if($type == "element"){
            if (!$is_sub_element)
                ...
                $name = JText::_(ucfirst($parent_shortcode)) ." ". JText::_('Item');
        }
        else{
            $name = $content;
        }
    ...
    public static function el_button($extra_, $config) {
        $icon = isset ($config['icon']) ? '<i class="ig-icon-formfields ig-' . $config['icon'] . '"></i>' : '';
