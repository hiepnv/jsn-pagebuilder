    ...
    $.JSNPbEditorHelper	= function (){
        ...
        JSNPbEditorHelper.initEntryButtons	= function (source_id, pbEntryButtons){
            ...
            $('.switchmode-button.pb-on', buttonGroups).on('click',  function (){
                if (!$(this).hasClass('active')) {
                    var new_content = window.parent.CKEDITOR.instances[source_id].getData();
            ...
            $('.switchmode-button.pb-off', buttonGroups).on('click',  function (){
                if (!$(this).hasClass('active')) {
                    ...
                    $('#' + source_id +' #cke_jform_content, #editor-xtd-buttons', container).addClass('editor-hidden').hide();
                    $('#' + source_id +', #editor-xtd-buttons', container).addClass('editor-hidden').hide();
                }
            });
            return buttonGroups;
        },
         ...
         */
            JSNPbEditorHelper.hideEditor	= function (container, source_id){
                $('#' + source_id + ', #cke_jform_articletext_, #editor-xtd-buttons', container).addClass('editor-hidden').hide();
                $('#' + source_id + ', #cke_jform_content_, #editor-xtd-buttons', container).addClass('editor-hidden').hide();
            },
        ...
        JSNPbEditorHelper.updateSource	= function(source){
            ...
            source.html(content);
            source.val(content);
            CKEDITOR.instances[source.attr('id')].setData(content);
