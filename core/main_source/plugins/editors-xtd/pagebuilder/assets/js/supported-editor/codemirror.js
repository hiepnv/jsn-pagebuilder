...
(function ($){
	$.JSNPbEditorHelper	= function (){
		...
		JSNPbEditorHelper.initEntryButtons	= function (source_id, pbEntryButtons){
			...
			$('.switchmode-button.pb-on', buttonGroups).on('click',  function (){				
				if (!$(this).hasClass('active')) {
					var new_content		= Joomla.editors.instances[source_id].getValue();
					// Set the new content from codeMirror to source textarea.
		...
		JSNPbEditorHelper.hideEditor	= function (container, source_id){			
			if ($('.CodeMirror-wrap', container).css('display') != 'none') {
				$('.CodeMirror-wrap', container).addClass('editor-hidden').hide();
			}else{
		...
		JSNPbEditorHelper.updateSource	= function(source){
            ...
            source.html(content);
            source.val(content);
            Joomla.editors.instances[source.attr('id')].setValue(content);
            setTimeout(function (){
            	Joomla.editors.instances[source.attr('id')].refresh();
            }, 0);
            return content;
		}
