...
(function ($) {
    ...
    $.Pb_Pricing_Item = function (options) {
        ...
        $('.submodal_frame_2').on('change', this.parameter, function (event) {
        ...
        $('body').on('jsnpb_changed', function(){

        	$('body').find('div.prtbl_item_attr_type').find('input[type="radio"]').bind('click', function(){
               if ($(this).is(':checked')) {
                   var textarea = $(this).closest('.jsn-item').first().find("[name^='shortcode_content']");
                   var shortcode_content = textarea.text();
                   var value = $(this).val();
