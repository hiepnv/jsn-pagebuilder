...
( function ($)
{
	...
	$.PBTable.prototype = {
        ...
        updateColWidth: function (active_shortcode, parent_wrap) {
        	...
        	var merge_data = parentIframe.contents().find('#param-width_value').val() + parentIframe.contents().find('#param-width_type option:selected').val();
            // update width
        ...
        spanProcess: function(parent_wrap){
            ...
            window.parent.jQuery.noConflict()('iframe').each(function () {
            ...
            $.each(extract_data.split('#'), function(key, value){
                if(value){
                    ...
                    if(attr_value <= 0){
                        alert(JSNPbParams.pbstrings.ROW_SPAN);
                        error = 1;
                    }
            ...
            switch (type) {
                    case 'column':
                        ...
                        if($("#table_content tr").last().find('td').length == 1){
                            alert(JSNPbParams.pbstrings.TABLE_COLUMNS);
                            return true;
                        }
                    ...
                    case 'row':
                        ...
                        if($("#table_content tr").length == 3){
                            alert(JSNPbParams.pbstrings.TABLE_ROWS);
                            return true;
                        }
        ...
        addRowCol:function(){
            ...
            $("#modalOptions").delegate(".table_action", "click", function(e){
                ...
                if($("#tmpl-"+$shortcode).length == 0){
            		$('body').append("<script type='text/html' id='tmpl-pb_table_item'>" + JSNPbParams.tpml_table_item + "</script>");
    ...
    $(document).ready(function(){
        ...
        $('body').bind('before_save_modal', function(e, shortcode) {
        	var table = new $.PBTable();
            table.init(shortcode);
        });
    })
